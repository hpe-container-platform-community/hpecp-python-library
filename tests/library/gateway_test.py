...
class TestGatewayGet(TestCase):
    ...
    def test_get_gateway(self, mock_get, mock_post):
        ...
        with self.assertRaisesRegexp(APIItemNotFoundException, "'gateway not found with id: /api/v1/workers/100'"):
...
class TestWaitForGatewayStatus(TestCase):
    ...
    def test_wait_for_status_gateway(self, mock_get, mock_post):
        ...
        with self.assertRaisesRegexp(AssertionError, "'timeout_secs' must be an int"):
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs='blah', state=[ GatewayStatus.ready ])

        with self.assertRaisesRegexp(AssertionError, "'timeout_secs' must be >= 0"):
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs=-1, state=[ GatewayStatus.ready ])

        with self.assertRaisesRegexp(AssertionError, "'state' item '0' is not of type GatewayStatus"):
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs=1, state=[ 'abc' ])

        self.assertTrue(
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs=1, state=[ GatewayStatus.installed ]))

        self.assertFalse(
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs=1, state=[ GatewayStatus.deleting ]))

        self.assertTrue(
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs=1, state=[ GatewayStatus.installed, GatewayStatus.deleting ]))

        self.assertFalse(
            get_client().gateway.wait_for_state(
                gateway_id='/api/v1/workers/123', timeout_secs=1, state=[ GatewayStatus.error, GatewayStatus.deleting ]))
